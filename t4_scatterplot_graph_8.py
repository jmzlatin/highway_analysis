...
def t4_tunnel_scatter_plot(df):
    ...
    try:
        ...
        t4_data = df[df['tunnel_or_interchange_number'] == 'T4']

        if t4_data.empty:
            st.warning(get_display("אין נתונים זמינים עבור מנהרת T4."))
            return
        ...
        ax.scatter(t4_data['date'], t4_data['hour_seconds'], color='green', s=50)
        ...
        ax.set_title(get_display("מנהרת T4"), fontsize=14, fontweight='bold')
